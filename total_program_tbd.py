...
mark2=pn.pane.Markdown("#### 본 프로그램에서 사용가능한 CAS넘버는 아래의 리스트에서 확인 가능합니다.", style={'font-family': 'NanumBarunGothic','font-size':'20px'})
...
############################################################
            ...
            # 그래프그릴 데이터 t_df
            line_gdf, stp_point_gdf, t_df, b_pec = rich.take_map_df(chemical,text_input8.value,stp_removal,water,df_e_t,Kdeg_water_per_hour)

            
            # line_gdf = gpd.GeoDataFrame(pd.merge(line_gdf, line_gdf_rtime.loc[:,['RCH_DID', 'rich_class']], how = 'left', on='RCH_DID'), geometry = 'geometry_x')
...
######################################### 3페이지 연산 ##################################################################################################################################
            ...
            #     return pn.Column(pn.pane.Markdown("### 음용수 섭취량", style={'font-family': 'NanumBarunGothic','font-size':'20px'}),pn.Row(fig1,fig2,fig3,fig4,fig5),mark2)
...
#######################################
            stream_chart_title=pn.pane.Markdown(" ### ■ 표준유역 거리기준 예측환경농도", style={'font-family': 'NanumBarunGothic','font-size':'20px'})
            
            @pn.depends(xx=select_area.param.value)
            def stream_chart(xx): 
                rich_plot=rich.get_stream_pec_plot(str(xx),chemical,text_input8.value,stp_removal,water,df_e_t,Kdeg_water_per_hour)
                return pn.Column(stream_chart_title,rich_plot)
            ...
            tabs.background="#ffffff"
    return tabs
mark_input=pn.pane.Markdown("#### * 계산 버튼을 누르기전에 <br> 입력사항을 확인해주세요", style={'font-family': 'NanumBarunGothic','font-size':'20px'})
